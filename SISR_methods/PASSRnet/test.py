...
def parse_args():
    ...
    parser.add_argument('--testset_dir', type=str, default='./data/test_part_lfo/')
...
def test(cfg):
    ...
    for idx in range(len(file_list)):
        ...
        # psnr_value3 = cal_psnr(hr0.cpu().detach().numpy(), SR_left[0].cpu().detach().numpy())

        psnr_value0 = PSNR(hr0.cpu().detach().numpy(), SR_left[0].cpu().detach().numpy())
